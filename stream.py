...
api_url = st.sidebar.text_input("URL de votre API FastAPI", "https://exam-ml-3.onrender.com/predict_file")
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if "predictions" in data:
                            ...
                            df['Statut'] = df['Prédiction'].apply(lambda x: "❌ Faux billet" if x==1 else "✅ Authentique")
                            
                            # Métriques
                            total = len(df)
                            faux = sum(df['Prédiction'])
                            auth = total - faux
                            ...
                            if total > 0:
                                fig_pie = px.pie(
                                    values=[auth, faux],
                                    names=['Authentiques', 'Faux billets'],
